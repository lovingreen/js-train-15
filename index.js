...
function roundObjectValues(obj) {
  ...
  const roundedValues = Object.entries(obj).map(([key, value]) => {
  ...
  return Object.fromEntries(roundedValues);
...
function calculateVolumeCylinder(radius, height) {
  ...
  return Math.ceil(Math.PI*radius*radius*height);
...
function sumPositiveNumbers(numbers) {
  if (!Array.isArray(numbers)) {
    return "Помилка: аргумент має бути масивом чисел"
  }
...
function getFractionalPart(num) {
  ...
  let fractionalPart = num - Math.trunc(num);
...
function estimateInvestment(principal, interestRate, years) {
  ...
  let investSum = principal * (1 + interestRate) ** years;
  return Math.round(investSum);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  let totalPrice = products.reduce((accum, item) => {
    accum += item.price;
    return accum;
  }, 0)
